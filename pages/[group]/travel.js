...
export default function TravelPage({ group }) {
  ...
  const tabClasses = (tab) =>
    `px-4 py-2 rounded cursor-pointer ${
      selected === tab ? 'bg-burgundy text-ivory' : 'text-black bg-navy/10 hover:bg-navy/10'
    }`;
                  ...
                  <details key={step.id} open className="group">
                    ...
                    <div className="space-y-3 mt-4">
                      {step.content.map((item, idx) => (
                        <details
                          key={idx}
                          className="group bg-ivory/30 rounded-lg border border-neutral/20"
                          open
                        >
            ...
            <div className="relative border-l-2 border-burgundy pl-6 space-y-8">
              ...
              {steps
                ...
                .map((step) => (
                  <div
                    key={step.id}
                    className="bg-ivory/20 rounded-lg p-6 border border-neutral/20"
                  >
                    <div className="mb-4">
                    ...
                    <div className="space-y-3">
                      {step.content.map((item, idx) => (
                        <details
                          key={idx}
                          className="group bg-ivory/40 rounded-lg border border-neutral/20"
                        >
